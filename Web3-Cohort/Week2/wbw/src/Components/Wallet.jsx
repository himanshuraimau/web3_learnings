...
const WalletGenerator = () => {
  ...
  return (
    <div className={`${theme === 'light' ? 'bg-gray-100 text-gray-800' : 'bg-gray-900 text-gray-100'} min-h-screen flex flex-col items-center justify-center font-sans p-4`}>
      <h1 className="mb-10 text-5xl font-extrabold">Web Wallet Generator</h1>

      <div className="w-full max-w-lg">
        <div className="flex flex-col items-center mb-10">
          <button
            onClick={generateMnemonic}
            className="w-full px-6 py-3 mb-4 text-lg text-white bg-green-600 rounded-lg hover:bg-green-700 transition duration-300 shadow-lg"
          >
            Generate Mnemonic
          </button>
          {mnemonic && (
            <div className="mt-4 p-4 rounded-lg bg-white dark:bg-gray-800 w-full shadow-lg border-l-4 border-green-500">
              <p className="italic text-gray-700 dark:text-gray-200">Mnemonic:</p>
              <p className="font-mono mt-2 mb-2 text-green-700 dark:text-green-400">{mnemonic}</p>
              <button
        ...
        <div className="mb-10 flex flex-col items-center">
          ...
          <button
            ...
            className="w-full px-6 py-3 text-lg text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition duration-300 shadow-lg"
          >
            Create Wallet
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {wallets.map((wallet, index) => (
            <div
              key={index}
              className={`p-4 rounded-lg ${theme === 'light' ? 'bg-white border border-gray-300' : 'bg-gray-800 border border-gray-600'} shadow-lg transition-transform transform hover:scale-105`}
            >
              ...
              <p className="font-mono mb-2 text-blue-600 dark:text-blue-400">{wallet.publicKey}</p>
              <button
              ...
              <p className="font-mono mb-2 text-blue-600 dark:text-blue-400">{wallet.address}</p>
              <button
        ...
        {generatedKey && (
          <div className="mt-10 p-4 rounded-lg bg-white dark:bg-gray-800 shadow-lg w-full border-l-4 border-red-500">
            <p className="text-lg font-bold text-gray-700 dark:text-gray-200">Private Key:</p>
            <p className="font-mono mt-2 mb-4 text-red-600 dark:text-red-400">{generatedKey}</p>
            <button
